...
export default function Loading({ errorLoading }) {
    ...
    const message = errorLoading ? "Something went wrong, try refreshing the page" : "Loading Posts"
    return (
        <div className={loadingStyles.spinnerContainer}>
            <FontAwesomeIcon
                className={errorLoading ? loadingStyles.spinnerFailed : loadingStyles.spinner}
                pulse={!errorLoading}
                icon={faSpinner}
            />
